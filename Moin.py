...
TELEGRAM_BOT_TOKEN = '7031779644:AAGD5fgJKtIQdodBmsszOf6NhmGRhW4jEik'
ADMIN_USER_ID = 1304907796
...
async def start(update: Update, context: CallbackContext):
    ...
    message = ( "*🚀 Use /attack <ip> <port> <duration>*\n"
    )
...
async def manage(update: Update, context: CallbackContext):
    ...
    if chat_id != ADMIN_USER_ID:
        await context.bot.send_message(chat_id=chat_id, text="*🔑 You need admin approval to use this command.*", parse_mode='Markdown')
        return
...
async def run_attack(chat_id, ip, port, duration, context):
    ...
    try:
        ...
        await context.bot.send_message(chat_id=chat_id, text=f"*❌ Error during the attack: {str(e)}*", parse_mode='Markdown')
